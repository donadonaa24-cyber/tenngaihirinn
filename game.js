    ...
    .map((id) => {
      ...
      return `
        <article class="prep-card ${roleClass}" data-char-id="${id}">
          ...
          <div class="prep-subtype">${base.role} / ${base.gender}</div>
...
function render() {
  ...
  if (els.cpuActionNotice) {
    els.cpuActionNotice.textContent = state.cpuNotice || "CPU待機中";
...
function renderBattleChannels() {
  ...
  if (els.playerFieldChannelBadge) {
    els.playerFieldChannelBadge.textContent = `プレイヤーチャンネル: ${playerChannelName}`;
  }
  if (els.enemyFieldChannelBadge) {
    els.enemyFieldChannelBadge.textContent = `CPUチャンネル: ${enemyChannelName}`;
  }
}
...
async function startTurn(side, options = {}) {
  ...
  setCpuNotice(isPlayerTurn ? "あなたのターンです。キャラを押して行動を選択してください。" : "CPUターン開始。行動を選択中…");
  if (showBanner) {
...
async function executeSkill(side, actorId, targets = {}) {
  ...
  switch (actorBaseId) {
    case "ao": {
      ...
      const recovered = heal(side, targetId, healValue);
    ...
    case "shiho": {
      ...
      allyTeam.field.forEach((allyId) => {
        totalRecovered += heal(side, allyId, healValue);
      });
      if (totalRecovered > 0) {
        playSfx("heal", { delayMs: followupDelayMs });
      }
    ...
    case "takumi": {
      ...
      const recovered = heal(side, targets.allyId, healValue);
      if (recovered > 0) {
        playSfx("heal", { delayMs: followupDelayMs });
      }
      break;
    ...
    case "yuzuki": {
      ...
      const recovered = heal(side, actorId, healValue);
      if (recovered > 0) {
        playSfx("heal", { delayMs: followupDelayMs });
      }
      break;
...
function deployEnemyBenchMember(reasonText = "") {
  ...
  log(`CPU補充: ${pickedCharacter.name} が控えから出撃。`);
  setCpuNotice(`CPU: ${pickedCharacter.name} を出撃。${reasonText}`.trim());
  return true;
}
...
async function maybeEnemyUseRevive() {
  ...
  const success = await rollChance(0.45, "enemy", "CPU 死者蘇生判定");
  if (!success) return;
...
async function runEnemyTurn() {
  ...
  try {
    ...
    if (!state.enemy.field.length) {
      setCpuNotice("CPUは出場可能なキャラがいないためターン終了。");
    ...
    for (const actorId of order) {
      if (state.gameOver || state.actionsUsed >= 3 || state.turn !== "enemy") break;
      ...
      if (actor.paralyzeTurns > 0) {
        ...
        setCpuNotice(`CPU: ${actor.name} は痺れで行動不能。`);
        await sleep(getEnemyActionDelay());
        continue;
      }
      ...
      if (!targetId) break;
      setCpuNotice(`CPU行動: ${actor.name} が通常攻撃。`);
...
async function endPlayerTurn() {
  ...
  setCpuNotice("ターン終了。CPUターンへ移行します…");
...
function setupEvents() {
  ...
  els.channelChoiceList.addEventListener("click", (event) => {
    ...
    void (async () => {
      ...
      renderBattleChannels();
      log(`チャンネル確定: プレイヤー「${channel.name}」 / CPU「${getChannelById(state.channels.enemy)?.name ?? "未選択"}」`);
